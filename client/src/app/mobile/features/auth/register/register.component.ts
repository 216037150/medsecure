    ...
    if (emailControl && emailControl.errors && this.emailTouched) {
      if (emailControl.errors['required']) {
        ...
        this.emailErrorMessage =
          'The email extension you entered appears to be invalid. Please use a valid email address (e.g., user@example.com)';
      } else {
        this.emailErrorMessage = 'Please enter a valid email address';
      }
    } else {
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid) {
      ...
      const existingUser = this.db
        .userTable()
        .find((user) => user.email === formValue.email);
      if (existingUser) {
        this.loading = false;
      ...
      const newUser: UserType = {
        ...
        role: 'PATIENT', // Always set to DOCTOR for desktop app
      };
      ...
      if (controls['fullname'].errors) {
        ...
        this.errorMessage = controls['confirmPassword'].errors[
          'passwordMismatch'
        ]
